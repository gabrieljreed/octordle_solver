# Wordle Solver Web UI ΓÇö Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a static web page version of the Wordle solver that mirrors the Qt desktop UI ΓÇö 6├ù5 tile grid, ranked best-guesses list, group breakdown panel, remaining words panel, dark mode, and copy-to-clipboard.
...
**`scoreGuess(guess, answer) ΓåÆ string`** (`solver.py:282`)
...
**`generateGroups(word, remainingWords) ΓåÆ Group[]`** (`solver.py:320`)
Scores `word` against every entry in `remainingWords`; buckets them by their score string. Returns one `Group` per unique score.

**`getAllAnswers(remainingWords, validGuesses) ΓåÆ AnswerPossibility[]`** (`solver.py:388`)
...
**`filterWords(remainingWords, guess, result) ΓåÆ string[]`** (`solver.py:231`)
...
Once a letter is committed (Enter pressed), clicking a tile cycles: GRAY ΓåÆ YELLOW ΓåÆ GREEN ΓåÆ GRAY.
Colors: GREEN `#69ab64`, YELLOW `#c3ae55`, GRAY `#767a7d`.

**Fitness sort** (`solver.py:95ΓÇô101` `__gt__`)
```
...
```
web/                              ΓåÉ all web-app code lives here
  package.json
  tsconfig.json
  vite.config.ts                  ΓåÉ base path + outDir ΓåÆ ../docs
  index.html
  style.css                       ΓåÉ layout, tiles, dark mode CSS vars
  src/
    types.ts                      ΓåÉ TileState, Group, AnswerPossibility, WorkerMessage
    solver.ts                     ΓåÉ scoreGuess, generateGroups, getAllAnswers, filterWords
    data.ts                       ΓåÉ loadData(): fetches word lists + best_second_guesses.json
    worker.ts                     ΓåÉ Web Worker: receives state, runs getAllAnswers, posts back
    grid.ts                       ΓåÉ Grid class: 6├ù5 DOM tiles, keyboard input, color cycling
    panels.ts                     ΓåÉ BestGuessList, GroupsPanel, RemainingWordsPanel
    app.ts                        ΓåÉ App: state, wires grid + panels + worker + dark mode
  public/
    data/
      valid_answers.txt           ΓåÉ copy of src/octordle_solver/data/valid_answers.txt
      valid_guesses.txt           ΓåÉ copy of src/octordle_solver/data/valid_guesses.txt
      best_second_guesses.json    ΓåÉ copy of src/octordle_solver/data/best_second_guesses.json
  tests/
    solver.test.ts                ΓåÉ unit tests for all pure solver functions
```
...
## Task 1 ΓÇö Scaffold the Project
...
- [ ] Write the failing tests first (`web/tests/solver.test.ts`):
  ...
  describe("scoreGuess", () => {
    ...
    it("handles duplicate letters ΓÇö only marks as many as exist in answer", () => {
...
- [ ] Run `npm test` from `web/` ΓÇö confirm all tests **fail** (solver.ts doesn't exist yet).
...
- [ ] Create `web/src/solver.ts`:
  ...
  /** Compare two AnswerPossibility objects: more groups wins; tie ΓåÆ smaller maxGroupSize wins. */
...
The `Grid` class manages the 6├ù5 tile DOM, keyboard input, and color cycling.

**Tile state machine per row:**
1. User types letters (AΓÇôZ): fills `Pending` tiles left-to-right.
2. Backspace: clears rightmost `Pending` tile.
3. Enter (when 5 letters filled): commits the row ΓÇö all tiles become `Absent` (gray), `letter_is_set = true`.
4. User clicks committed tiles: cycles Absent ΓåÆ Misplaced ΓåÆ Correct ΓåÆ Absent.

- [ ] Add to `web/index.html` inside `<div id="app">`:
  ```html
  <header>
    <h1>Wordle Solver</h1>
    <button id="dark-mode-toggle" aria-label="Toggle dark mode">≡ƒîÖ</button>
  </header>
  <main>
    <div id="grid"></div>
    <div id="sidebar">
...
- [ ] Start dev server, open browser, confirm the 6├ù5 grid renders. Type a few letters, press Enter, click tiles to cycle colors. Stop server.

- [ ] Commit: `git add web/src/grid.ts web/style.css web/index.html && git commit -m "feat(web): add tile grid component"`

---

## Task 8 ΓÇö Panels (Best Guesses + Groups + Remaining Words)
...
- `remainingWords: string[]` ΓÇö starts as all valid answers
- `validGuesses: string[]`
- `bestSecondGuesses: Record<string, string>`
- `guesses: Guess[]` ΓÇö each committed + colored guess
...
4. Disable the button and show "CalculatingΓÇª".
...
Key = result string with `YΓåÆ0, MΓåÆ1, NΓåÆ2` substituted: e.g. `"YNNNN"` ΓåÆ `"02222"`.
...
- [ ] Create `web/src/app.ts`:
  ...
  class App {
    ...
    private onGetGuesses(): void {
      ...
      this.getGuessesBtn.textContent = "CalculatingΓÇª";
...
- [ ] Run `npm run dev`. Do a full smoke test:
  ...
  4. Button shows "CalculatingΓÇª" then re-enables; best guesses list populates.
  5. Click a guess in the list ΓÇö groups panel populates.
  6. Double-click a guess ΓÇö grid fills with that word.
  7. Click "Copy" ΓÇö clipboard contains remaining words.
  8. Click dark-mode toggle ΓÇö colors flip.
  9. Click "Reset" ΓÇö everything clears.

- [ ] Commit: `git add web/src/app.ts web/index.html && git commit -m "feat(web): wire up app controller"`

---

## Task 10 ΓÇö Final Styling Pass
...
## Task 11 ΓÇö Build and Deploy to GitHub Pages

**Files:**
- Modify: `web/vite.config.ts` (set correct repo name)

- [ ] Edit `web/vite.config.ts` ΓÇö replace `"octordle_solver"` in `base` with your actual GitHub repo name (check with `git remote -v`).
...
- [ ] On GitHub: Settings ΓåÆ Pages ΓåÆ Source = "Deploy from branch", Branch = `main`, Folder = `/docs`. Save.

- [ ] Visit `https://<username>.github.io/<repo>/` ΓÇö confirm the page loads and the solver works.
...
| Click tile ΓåÆ cycles colors | GRAY ΓåÆ YELLOW ΓåÆ GREEN ΓåÆ GRAY |
...
| First guess Γëá SLATE | No pinned second guess |
